...
- #685: Websocket connection cancellation no longer cancels the background receive loop after a
  successful connect.
- #690: `ServerDisconnect` is only raised once for a disconnect.
- #692: Negative rotate percentages are accepted for reverse rotation.
- #693: Device and all-device stop helpers now send v4 `StopCmd` messages.
- Clean up failed client connections after device-list handshake failures.
- Client scan and stop helpers now throw `ButtplugClientConnectorException` when called before
  connecting instead of throwing `NullReferenceException`.
